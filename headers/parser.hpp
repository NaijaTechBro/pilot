...
struct AST_NODE {
    ...
    std::string * VALUE;
    AST_NODE * CHILD;
    std::vector <AST_NODE *> SUB_STATEMENT; // For the root node
};
...
public:
    ...
    void proceed(enum type TYPE) {
        if (current->TYPE != TYPE) {
            std::cout << "[!] SYNTAX ERROR: Expected " << typeToString(TYPE)
                      << " but got " << typeToString(current->TYPE) << std::endl;
            exit(1);
        }

        index++;
        // CRITICAL: Only access the vector if we are still within bounds
        if (index < limit) {
            current = parserTokens.at(index);
        }
    }
    ...
    AST_NODE * parseINT() {
        if (current->TYPE != TOKEN_INT) {
            std::cout << "[!] SYNTAX ERROR: Expected Integer" << std::endl;
            exit(1);
        }
    ...
    AST_NODE * parseID() {
        // We don't need the buffer right now, but good for future expansion
        proceed(TOKEN_ID);
        ...
        // Semicolon is handled by the main loop's proceed(TOKEN_SEMICOLON)
        return newNode;
    }

    AST_NODE * parseRETURN() {
        proceed(TOKEN_KEYWORD); // consumes "return"
        AST_NODE * newNode = new AST_NODE();
    ...
    AST_NODE * parse() {
        ...
        while (index < limit && current->TYPE != TOKEN_EOF) {
            switch (current->TYPE) {
                case TOKEN_ID:
                    ROOT->SUB_STATEMENT.push_back(parseID());
                    break;
                case TOKEN_KEYWORD:
                    ROOT->SUB_STATEMENT.push_back(parseKEYWORD());
                    break;
                default:
                    std::cout << "[!] SYNTAX ERROR: Unexpected token " << typeToString(current->TYPE) << std::endl;
                    exit(1);
            }
            // Consumes the ';' after every statement
